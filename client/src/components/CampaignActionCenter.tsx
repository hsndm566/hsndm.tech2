...
export function CampaignActionCenter({ applicationStatuses = [], hasCandidateApproval, isArabic = false, verifiedEvidenceCount }: Props) {
  ...
  return (
    <section aria-labelledby="campaign-action-center-title" className="rounded-3xl border border-[#e5482a]/30 bg-white p-5 shadow-sm sm:p-7" dir={isArabic ? "rtl" : "ltr"}>
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        ...
        <a className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-xl bg-[#151515] px-4 py-2.5 text-sm font-extrabold text-[#f5f2eb] transition hover:bg-[#e5482a] hover:text-[#151515] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#e5482a] focus-visible:ring-offset-2" href={action.href}>{action.cta}<ArrowRight className={`size-4 ${isArabic ? "-scale-x-100" : ""}`} /></a>
      </div>
...
function ActionMetric({ icon, label, positive, value }: { icon: React.ReactNode; label: string; positive: boolean; value: string }) {
  return <div className="rounded-2xl border border-[#151515]/10 bg-[#fdfcf9] p-4"><span className={`grid size-8 place-items-center rounded-lg ${positive ? "bg-emerald-100 text-emerald-800" : "bg-[#e5482a]/10 text-[#e5482a]"}`}>{positive ? <CheckCircle2 className="size-4" /> : icon}</span><p className="mt-3 font-mono text-[10px] uppercase tracking-[.12em] text-stone-500">{label}</p><p className="mt-1 text-sm font-bold text-[#151515]">{value}</p></div>;
}
