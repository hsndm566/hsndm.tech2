...
import { createLocalAtsReview, type AtsReview } from "@/lib/localAts";
...
export default function Ats() {
  ...
  const exportResult = () => {
    if (!review) return;
    const content = `AutoApply SA ATS review\n\nScore: ${review.score}/100\n\n${review.summary}\n\nStrengths\n${review.strengths.map(x => `- ${x}`).join("\n")}\n\nPriority improvements\n${review.gaps.map(x => `- ${x}`).join("\n")}\n\nOptimized CV bullets\n${review.optimizedBullets.map(x => `- ${x}`).join("\n")}\n\n${review.disclaimer}`;
      ...
      <div className="mx-auto max-w-3xl space-y-7">
        ...
        {review && (
          <section className="space-y-4 bg-[#151515] p-6 text-white">
            ...
            <p>{review.summary}</p>
            <h3>Strengths</h3><ul>{review.strengths.map(item => <li key={item}>• {item}</li>)}</ul>
            <h3>Priority improvements</h3><ul>{review.gaps.map(item => <li key={item}>• {item}</li>)}</ul>
            <h3>Suggested bullets</h3><ul>{review.optimizedBullets.map(item => <li key={item}>• {item}</li>)}</ul>
            <small>{review.disclaimer}</small>
            <div className="space-y-3 border-t border-white/25 pt-4">
              <div className="my-4 border border-white/20 bg-white/10 p-4">{review.score < 70 ? <div className="space-y-2"><p className="text-sm font-medium">Fixed your CV? Check your new score →</p><label className="inline-block cursor-pointer bg-white px-4 py-2 text-sm font-medium text-[#151515] hover:bg-white/90"><input className="sr-only" type="file" accept=".pdf,.docx,.txt" onChange={event => choose(event.target.files?.[0])} />Upload revised CV & re-analyse</label></div> : <div className="space-y-2"><p className="text-sm font-medium">You&apos;re ready. Start a campaign →</p><Link href="/#upload" className="inline-block bg-[#e5482a] px-4 py-2 text-sm font-medium text-white">Start a campaign on homepage</Link></div>}</div>
