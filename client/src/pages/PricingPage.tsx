...
type PageCopy = {
  ...
  comparisonKicker: string; comparisonTitle: string; oneApplication: string; swipeToCompare: string; comparisonRegion: string;
...
export default function PricingPage({ language = "en" }: { language?: "en" | "ar" }) {
  ...
  return <main className="min-h-screen bg-[#f3f0e9] text-[#151515]" dir={arabic ? "rtl" : "ltr"} lang={language}>
    ...
    <section className="page-frame py-16">
      ...
      <section className="mt-16" aria-labelledby="comparison-title"><p className="font-mono text-xs text-[#e5482a]">{copy.comparisonKicker}</p><h2 id="comparison-title" className="mt-3 text-3xl font-bold tracking-tight">{copy.comparisonTitle}</h2><p className="mt-4 max-w-3xl text-sm leading-6 text-black/70">{copy.oneApplication}</p><p id="comparison-instructions" className="mt-3 text-xs font-mono text-black/60">↔ {copy.swipeToCompare}</p><div className="mt-4 overflow-x-auto border border-black/10 bg-white outline-none focus-visible:ring-2 focus-visible:ring-[#e5482a] focus-visible:ring-offset-2" tabIndex={0} role="region" aria-label={copy.comparisonRegion} aria-describedby="comparison-instructions" onKeyDown={scrollComparison}><table className={`min-w-[720px] w-full border-collapse text-sm ${arabic ? "text-right" : "text-left"}`}><thead className="bg-[#151515] text-white"><tr><th className="p-4 font-mono text-xs">{arabic ? "المعيار" : "Comparison"}</th>{plans.map((plan) => <th key={plan.name} className="p-4 font-mono text-xs">{plan.name}<span className="mt-1 block text-white/60">{plan.price} SAR</span></th>)}</tr></thead><tbody>{comparisonRows.map((row, index) => <tr key={row.en} className={index % 2 ? "bg-[#fbf9f5]" : "bg-white"}><th scope="row" className="border-t border-black/10 p-4 font-semibold">{arabic ? row.ar : row.en}</th>{(arabic ? row.valuesAr : row.values).map((value, valueIndex) => <td className="border-t border-black/10 p-4 align-top text-black/70" key={`${row.en}-${valueIndex}`}>{value}</td>)}</tr>)}</tbody></table></div></section>
