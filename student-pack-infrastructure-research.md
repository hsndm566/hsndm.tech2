...
Claim the Clerk Student plan immediately because it directly matches the existing passwordless dashboard authentication. For the deployment target, Heroku is the best Student Pack fit for the current Node/Express/tRPC dashboard API: it can deploy directly from GitHub and supports a single Node service without an application-server rewrite. The current project uses Drizzle's MySQL driver and a `DATABASE_URL`, so the initial Heroku deployment should retain the existing compatible database rather than migrate data to Heroku Postgres. A valid card is required, overages are billable, unused credit does not roll over, and the offer is limited to 24 months.
