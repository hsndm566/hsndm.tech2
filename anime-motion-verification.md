...
- **Client-side language transition:** A normal-motion DevTools session navigated from `/` to `/ar` without reloading. The controller stayed in `full` mode, rebound from nine English word targets to six Arabic word targets, and the Arabic hero remained visible.
