...
describe("dashboard authentication monitor", () => {
  it("marks a non-successful Supabase health as degraded without candidate data", async () => {
    ...
    expect(check.supabaseHealthStatus).toBe(403);
  ...
  it("uses a redacted Sentry envelope without request or candidate payloads", async () => {
    ...
    const reported = await reportAuthMonitorToSentry({ status: "degraded", configurationReady: true, supabaseHealthStatus: 403 }, false, send as unknown as typeof fetch);
