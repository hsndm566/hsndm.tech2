...
export async function analyzeAts(input: z.infer<typeof atsInput>) {
  const messages = [{ role: "system" as const, content: "You are a careful ATS advisor for Saudi Arabia job seekers. Analyse only the supplied CV text. Do not invent qualifications, employers, metrics, or job outcomes. Return complete JSON only. Keep the review concise: no more than 4 strengths, 4 gaps, and 5 optimized bullets; keep each item under 18 words. State that this is guidance, not a hiring prediction." }, { role: "user" as const, content: `Target role: ${input.targetRole || "Not specified"}\n\nCV text:\n${input.cvText}` }];
  let content: string | undefined;

  if (isGroqConfigured()) {
    try {
      content = await invokeGroqJson({ messages, maxCompletionTokens: 1000 });
    } catch {
      // Preserve ATS availability if Groq is temporarily unavailable.
    }
  }
